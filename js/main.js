...
function startAutoRunLoop() {
    ...
    const update = () => {
        ...
        if (depth >= max) { depth = max; autoRunDirection = -1; }
        else if (depth <= min) { depth = min; autoRunDirection = 1; }
...
function toggleAutoRun() {
    ...
    else clearInterval(autoRunTimer);
}

btnAutoRun.onclick = toggleAutoRun;
speedSlider.oninput = () => { if (autoRunActive) startAutoRunLoop(); };
